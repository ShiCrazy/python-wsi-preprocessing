...
def generate_tile_summaries(tile_sum, np_img, display=True, save_summary=False):
  ...
  """
    ...
    original_img_path = slide.get_training_image_path(slide_num, slide_filepath)
...
def generate_good_tile_summaries(tile_sum, np_img, display=True, save_summary=False, show_good_stats=False,
    ...
    summary_title = "Slide %03d Good Tile Summary:" % slide_num
...
def generate_top_tile_summaries(tile_sum, np_img, display=True, save_summary=False, show_top_stats=True,
  ...
  Args:
    ...
    border_all_tiles: If True, border all tiles. If False, border only top files.
  """
    z = 300  # height of area at top of summary slide
    slide_num = tile_sum.slide_num
    slide_filepath = tile_sum.slide_filepath
    ...
    if display:
        ...
        summary_orig.show()
    if save_summary:
        save_top_tiles_image(summary, slide_num, slide_filepath)
        save_top_tiles_on_original_image(summary_orig, slide_num, slide_filepath)
...
def save_tile_summary_image(pil_img, slide_num, slide_filepath):
  ...
  """
    t = Time()
    filepath = slide.get_tile_summary_image_path(slide_num, slide_filepath)
    pil_img.save(filepath)
    print("%-20s | Time: %-14s  Name: %s" % ("Save Tile Sum", str(t.elapsed()), filepath))

    # t = Time()
    # thumbnail_filepath = slide.get_tile_summary_thumbnail_path(slide_num, slide_filepath)
...
def save_good_tiles_image(pil_img, slide_num, slide_filepath):
    """
  Save a good tiles image and thumbnail to the file system.
  ...
  """
    t = Time()
    filepath = slide.get_good_tiles_image_path(slide_num, slide_filepath)
    pil_img.save(filepath)
    ...
    # slide.save_thumbnail(pil_img, slide.THUMBNAIL_SIZE, thumbnail_filepath)
    # print("%-20s | Time: %-14s  Name: %s" % ("Save Good Tiles Thumb", str(t.elapsed()), thumbnail_filepath))


def save_top_tiles_image(pil_img, slide_num, slide_filepath):
    """
  ...
  """
    t = Time()
    filepath = slide.get_top_tiles_image_path(slide_num, slide_filepath)
    pil_img.save(filepath)
...
def save_tile_summary_on_original_image(pil_img, slide_num, slide_filepath):
  ...
  """
    t = Time()
    filepath = slide.get_tile_summary_on_original_image_path(slide_num, slide_filepath)
    pil_img.save(filepath)
...
def save_good_tiles_on_original_image(pil_img, slide_num, slide_filepath):
    """
  Save a good tiles on original image and thumbnail to the file system.
  ...
  """
    t = Time()
    filepath = slide.get_good_tiles_on_original_image_path(slide_num, slide_filepath)
    pil_img.save(filepath)
    print("%-20s | Time: %-14s  Name: %s" % ("Save Good Orig", str(t.elapsed()), filepath))

    # t = Time()
    # thumbnail_filepath = slide.get_good_tiles_on_original_thumbnail_path(slide_num)
    # slide.save_thumbnail(pil_img, slide.THUMBNAIL_SIZE, thumbnail_filepath)
    # print(
    #     "%-20s | Time: %-14s  Name: %s" % ("Save Good Orig Thumb", str(t.elapsed()), thumbnail_filepath))


def save_top_tiles_on_original_image(pil_img, slide_num, slide_filepath):
    """
  ...
  """
    t = Time()
    filepath = slide.get_top_tiles_on_original_image_path(slide_num, slide_filepath)
    pil_img.save(filepath)
...
def summary_and_tiles(slide_num, slide_filepath, display=False, save_summary=True, save_data=True,
  ...
  Args:
    ...
    save_good_tiles: If True, save good tiles to files.

  """
    img_path = slide.get_filter_image_result(slide_num, slide_filepath)
    ...
    generate_top_tile_summaries(tile_sum, np_img, display=display, save_summary=False)
    if save_top_tiles:
...
def save_tile_data(tile_summary):
  ...
  """
    ...
    data_path = slide.get_tile_data_path(tile_summary.slide_num, tile_summary.slide_filepath)
...
def tile_to_pil_tile(tile):
  ...
  """
    ...
    s = slide.open_slide(t.slide_filepath)

    x, y = t.o_c_s, t.o_r_s
    w, h = t.o_c_e - t.o_c_s, t.o_r_e - t.o_r_s
    if w == h:  # 切片的长与宽理应相等，还没有测试，请注意
    ...
    if tile_pil_img is not None:
        ...
        if save:
            ...
            if not os.path.exists(dir):
                os.makedirs(dir)
            tile_pil_img = tile_pil_img.resize((256, 256)) # ！！！！！！！！为了适应HIPT，将切片的大小改为256*256，以实现在20倍放大倍数下patch的效果！！！！！！！！！！！
...
def image_range_to_tiles(start_ind, end_ind, display=False, save_summary=False, save_data=False, save_top_tiles=False,
  ...
  """
    ...
    for slide_num in range(start_ind, end_ind + 1):
        slide_filepath = os.path.join(slide.SRC_TRAIN_DIR, os.listdir(slide.SRC_TRAIN_DIR)[slide_num - 1])
        if slide_filepath[-4:] != ".svs":
            continue
        print(slide_filepath.split("\\")[-1][:12])
