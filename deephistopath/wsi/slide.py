...
def get_training_slide_path(slide_number):
  ...
  Convert slide number to a path to the corresponding WSI training slide file.
    将slide编号转换为对应到WSI训练slide文件的路径上
...
def get_tile_image_path(tile):
  ...
  pixel width, and pixel height.
    基于tile的有关信息获取tile图像的路径，例如行，列，行像素位置，列像素位置，像素宽，像素高
  ...
  """
    ...
    slide_name = str(slide_filepath).split("\\")[-1][:12]
    tile_path = os.path.join(TILE_DIR, padded_sl_num + "-" + slide_name,
...
def get_tile_image_path_by_slide_row_col(slide_number, row, col):
  ...
  Obtain tile image path using wildcard lookup with slide number, row, and column.
    使用slide的编号、行和列通配符查找的方式获取tile图像路径
...
def get_training_image_path(slide_number, slide_filepath, large_w=None, large_h=None, small_w=None, small_h=None):
  ...
  the corresponding file based on the slide number will be looked up in the file system using a wildcard.
    将slide编号和可选维度转化到一个训练图像的路径中。如果没有输入维度，基于slide编号的对应文件将以通配符的形式在文件系统中被查找
  ...
  """
    ...
    slide_name = slide_filepath.split("\\")[-1][0:12]
...
def get_training_thumbnail_path(slide_number, slide_filepath, large_w=None, large_h=None, small_w=None, small_h=None):
    """
  ...
  supplied, the corresponding file based on the slide number will be looked up in the file system using a wildcard.
    将slide编号和可选维度转化到一个训练缩略图的路径中。如果没有输入维度，基于slide编号的对应文件将以通配符的形式在文件系统中被查找
  ...
  """
    ...
    slide_name = slide_filepath.split("\\")[-1][0:12]
    if large_w is None and large_h is None and small_w is None and small_h is None:
        wilcard_path = os.path.join(DEST_TRAIN_THUMBNAIL_DIR,
                                    TRAIN_PREFIX + padded_sl_num + "-" + slide_name + "*." + THUMBNAIL_EXT)
        img_path = glob.glob(wilcard_path)[0]
    else:
        img_path = os.path.join(DEST_TRAIN_THUMBNAIL_DIR, TRAIN_PREFIX + padded_sl_num + "-" + slide_name + "-" + str(
            SCALE_FACTOR) + "x-" + DEST_TRAIN_SUFFIX + str(
...
def get_filter_image_path(slide_number, filter_number, filter_name_info):
  ...
  Convert slide number, filter number, and text to a path to a filter image file.
    将slide编号、filter编号和文本转化到一个filter图像文件中
...
def get_filter_thumbnail_path(slide_number, filter_number, filter_name_info):
  ...
  Convert slide number, filter number, and text to a path to a filter thumbnail file.
    将slide编号、filter编号和文本转化到一个filter缩略图文件的路径中
...
def get_filter_image_filename(slide_number, filter_number, filter_name_info, thumbnail=False):
  ...
  Convert slide number, filter number, and text to a filter file name.
    将slide编号、filter编号和文本转化到一个filter的文件名中
...
def get_tile_summary_thumbnail_path(slide_number, slide_filepath):
    """
  Convert slide number to a path to a tile summary thumbnail file.
    将slide编号转化到指向tile总结缩略图文件的路径中
  ...
  """
    ...
    img_path = os.path.join(TILE_SUMMARY_THUMBNAIL_DIR,
                            get_tile_summary_image_filename(slide_number, slide_filepath, thumbnail=True))
    return img_path
...
def get_tile_summary_on_original_image_path(slide_number, slide_filepath):
  ...
  Convert slide number to a path to a tile summary on original image file.
    将slide编号转化到指向基于原始图像文件的tile总结路径中
...
def get_good_tiles_on_original_image_path(slide_number, slide_filepath):
    """
  Convert slide number to a path to a good tiles on original image file.

  Example:
    5 -> ../data/good_tiles_on_original_png/TUPAC-TR-005-32x-49920x108288-1560x3384-good_tiles.png

  Args:
    slide_number: The slide number.

  Returns:
    Path to the good tiles on original image file.
  """
...
def get_top_tiles_on_original_image_path(slide_number, slide_filepath):
    """
  ...
  """
    ...
    img_path = os.path.join(TOP_TILES_ON_ORIGINAL_DIR, get_good_tiles_image_filename(slide_number, slide_filepath))
    return img_path


def get_top_tiles_on_original_thumbnail_path(slide_number, slide_filepath):
    """
  ...
  """
    ...
    img_path = os.path.join(TOP_TILES_ON_ORIGINAL_THUMBNAIL_DIR,
                            get_top_tiles_image_filename(slide_number, slide_filepath, thumbnail=True))
    return img_path
...
def get_good_tiles_image_filename(slide_number, slide_filepath, thumbnail=False):
    """
  Convert slide number to a good tiles image file name.

  Example:
    5, False -> TUPAC-TR-005-32x-49920x108288-1560x3384-good_tiles.png
    5, True -> TUPAC-TR-005-32x-49920x108288-1560x3384-good_tiles.jpg
  ...
  Returns:
    The good tiles image file name.
  """
    if thumbnail:
        ext = THUMBNAIL_EXT
    else:
...
def get_top_tiles_image_filename(slide_number, slide_filepath, thumbnail=False):
    """
...
def get_good_tiles_image_path(slide_number, slide_filepath):
    """
  Convert slide number to a path to a good tiles image file.

  Example:
    5 -> ../data/good_tiles_png/TUPAC-TR-005-32x-49920x108288-1560x3384-good_tiles.png

  Args:
    slide_number: The slide number.

  Returns:
    Path to the good tiles image file.
  """
...
def get_top_tiles_image_path(slide_number, slide_filepath):
    """
  ...
  """
    ...
    img_path = os.path.join(TOP_TILES_DIR, get_top_tiles_image_filename(slide_number, slide_filepath))
    return img_path


def get_top_tiles_thumbnail_path(slide_number, slide_filepath):
    """
  Convert slide number to a path to a tile summary thumbnail file.
  ...
  """
    ...
    img_path = os.path.join(TOP_TILES_THUMBNAIL_DIR,
                            get_top_tiles_image_filename(slide_number, slide_filepath, thumbnail=True))
    return img_path
...
def training_slide_to_image(slide_number, slide_filepath):
  ...
  Convert a WSI training slide to a saved scaled-down image in a format such as jpg or png.
    将一张WSI训练slide转换为一张以jpg或png格式保存的缩小的图片
  Args:
    slide_number: The slide number.
    slide_filepath: The slide filepath
  """
    ...
    # thumbnail_path = get_training_thumbnail_path(slide_number, slide_filepath, large_w, large_h, new_w, new_h)
...
def show_slide(slide_number):
  ...
  展示一张WSIslide，slide已被缩小并转化为PIL图像
  Args:
    slide_number: The slide number.
  """
...
def multiprocess_training_slides_to_images():
  ...
  用多进程将所有的WSI训练slide转换为更小的图像，每个进程会处理一系列的slide
  """
    timer = Time()
